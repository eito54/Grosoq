...
**Q: 他の言語に対応していますか？**
A: 現在は日本語・英語・フランス語(Français)・スペイン語(Español)に対応しており、アプリ内で動的に切り替え可能です。
...
- **v4.1.0**: 校正プリセット・接続品質表示・多言語拡充
  - **ゲーム別校正プリセット**: MK8DX / MK World 12人 / MK World 24人 の3枠固定プリセット。解析モードとゲームに応じて自動切替
  - **標準12人モードのMK World対応**: 操作画面でMK8DX / MK Worldを切替可能。MK World 12人ではリザルト全員の領域を単一クロップして解析
  - **OBS接続品質表示・自動再接続**: RTT計測による品質バッジ（Good/Fair/Laggy）、再接続状況の表示
  - **レース数の推定表示**: 合計点から現在レース数を推定（切断者等の欠損時は「推定」と表示）
  - **フランス語・スペイン語対応**（日本語/English/Français/Español）
  - **設定ファイルの暗号化**（OSの資格情報ストレージ使用・WindowsはDPAPI）、設定保存のアトミック化
  - **オーバーレイのアニメーション改善**: 更新が止まる不具合の修正、滑らかなカウントアップ、モーション低減設定対応

- **v4.0.0**: 24人スタンドモード・MKWorld対応・完全英語化
  - **24人スタンド読取モード**: 列ごとの校正UI（プレビュー付き）で名前/点数領域を精密指定、ステッカー誤認識を防止
  - **dc対策**: 切断・再入室でスコアが0に戻っても確認ダイアログから前回値を復元、以後自動加算
  - **変化ハイライト・欠席プレイヤー保持**
  - **解析モデルをQwen 3.6 27Bに切替**（思考モード無効化で高速化）
  - **UI完全英語対応**（日本語/英語切替ですべての項目が切り替わります）
  - **デザイン刷新**: レーシングアクセント配色、ネストスクロール解消、コントラスト改善
